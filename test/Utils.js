...
assert.hasLog = function(result, event, params, match=null) {
  const logParams = {};

  const logOrLogs = result.events[event]; // This, awkwardly, is either a log or an array
  ...
  if (Array.isArray(logOrLogs)) {
    if (match) {
      ...
      log = logOrLogs.find(l => l.returnValues[matchKey] === matchValue);

      if (!log) {
        const values = logOrLogs.map(l => l.returnValues[matchKey]);
        assert.fail(0, 1, `Expected log with event \`${event}\` and ${matchKey}=${matchValue}, found logs with values: [${values}]`);
      }
    } else {
      assert.fail(0, 1, `Expected log with event \`${event}\`, found multiple matching logs. Please add a match key`);
    }
  } else {
    log = logOrLogs;
  }
