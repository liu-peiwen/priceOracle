...
const infuraNetworks = networks.reduce((networks, network) => {
  ...
  if (networksHome && !privateKeyHex) {
    try {
      let postfix = '';
      if (process.env.ETHEREUM_KEY_POSTFIX) {
        postfix = `-${process.env.ETHEREUM_KEY_POSTFIX}`;
      }

      // Try to read from file
      const networkPathResolved = path.join(fs.realpathSync(networksHome), `${network}${postfix}`);
